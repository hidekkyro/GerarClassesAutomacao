...
BEGIN
	...
	BEGIN
		...
		INSERT INTO #tmp_Result (tipo,nomeArquivo,extensaoArquivo,conteudoArquivo) VALUES ('Classe',
	'' + @tabelaProcessando + '',
	'cs',
	'
...
INSERT INTO #tmp_Result (tipo,nomeArquivo,extensaoArquivo,conteudoArquivo) VALUES ('Mapeamento',
	'' + @tabelaProcessando + 'Map',
	'cs',
'using Domain.Entities.Local.Documentos;
...
')
	...
	SET @linhas += CHAR(9) + CHAR(9) + CHAR(9) + 'builder.Property(x => x.' + @nomeColuna + ').HasColumnType("' + UPPER(@tipo) + '")'
	IF(@tipo = 'varchar')
	BEGIN
	...
	select 
		@foreignMap += CHAR(9) + CHAR(9) + CHAR(9) + 'builder.HasOne(x => x.' + pk_tab.name + ').WithMany('+ LOWER(SUBSTRING(pk_tab.name,1,1)) +' => '+ LOWER(SUBSTRING(pk_tab.name,1,1)) +'.'+ @nomeTabela +'s).HasForeignKey('+ LOWER(SUBSTRING(@nomeTabela,1,1)) +' => '+ LOWER(SUBSTRING(@nomeTabela,1,1)) +'.'+ @nomeColuna +');'+ CHAR(13)+CHAR(10)
	...
	*/
	...
	SET @classeConstrutor += CHAR(9) +CHAR(9) + CHAR(9) + @nomeColuna + ' = ' + LOWER(SUBSTRING(@nomeColuna,1,1)) + SUBSTRING(@nomeColuna,2,LEN(@nomeColuna)) + ';' + CHAR(13)+CHAR(10)

	SET @classeAtributos += CHAR(9) +CHAR(9) + 'public ' + @classeTipoVarConvertido + ' ' + @nomeColuna + ' { get; private set; }' + CHAR(13)+CHAR(10)

	SET @classeMetodo += CHAR(9) +CHAR(9) + CHAR(9) + @nomeColuna + ' = entidade.' + @nomeColuna + ';' + CHAR(13)+CHAR(10)
...
SELECT --ROW_NUMBER() OVER(ORDER BY conteudoArquivo) Id,
		tipo,
       nomeArquivo,
       extensaoArquivo,
       conteudoArquivo 
FROM #tmp_Result FOR XML RAW ('Itens'), root ('CriacaoClasse')
